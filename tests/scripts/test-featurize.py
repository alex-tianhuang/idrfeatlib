...
def test_no_regions(tmp_path):
    ...
    with open(inf, "w") as file:
        for protid, seq in input_sequences:
            file.write(">{}\n{}\n".format(protid, seq))
    run_script("featurize.py", [inf, outf])
    with open(outf, "r") as file:
        ...
        for row in rows:
            assert row[proteinid_col] in test_protids
            for feat_name in featurenames:
                feat_value = row[feat_name]
                try:
                    float(feat_value)
                except ValueError:
                    assert isinstance(feat_value, str) and (not feat_value)
...
def test_with_regions(tmp_path):
    ...
    run_script("featurize.py", [infa, "--input-regions", inrgns, outf])
    with open(outf, "r") as file:
        reader = csv.DictReader(file)
        rows = list(reader)
        assert reader.fieldnames is not None
        ...
        for row in rows:
            assert (row[proteinid_col], row[regionid_col]) in test_ids
            for feat_name in featurenames:
                feat_value = row[feat_name]
                try:
                    float(feat_value)
...
def test_custom_feature_json(tmp_path):
    ...
    run_script("featurize.py", [infa, outf, "--feature-file", injson])
    with open(outf, "r") as file:
        reader = csv.DictReader(file)
        rows = list(reader)
        assert reader.fieldnames is not None
